...
class RequestHelper {
  ...
  Future<Map<String, dynamic>> getActiveHours({
    ...
    try {
      final data = await _dioClient.get(
        Endpoints.getAppointmentTimeStamp,
        queryParameters: queryParameters,
        cancelToken: cancelToken,
      );
  ...
  Future<Map<String, dynamic>> getAppointmentProduct({
    ...
    try {
      final data = await _dioClient.get(
        "${Endpoints.getAppointmentProduct}/$productId",
        queryParameters: queryParameters,
        cancelToken: cancelToken,
      );
      return data is Map ? data.map((key, value) => MapEntry(key.toString(), value)) : {};
  ...
  /// Add to cart
  Future<Map<String, dynamic>> addToCart({required Map<String, String?> queryParams, Map<String, dynamic>? data}) async {
    try {
