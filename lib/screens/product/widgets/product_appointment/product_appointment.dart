...
class _ProductAppointmentState extends State<ProductAppointment> with Utility {
  ...
  final ValueNotifier<StaffModel> _pickedStaff =
      ValueNotifier(StaffModel(name: "No Preference"));
    ...
    if (availabilityAutoSelect) {
      await Future.doWhile(() async {
        ...
        return (records.isEmpty &&
            !endDate.isAfter(DateTime.now().add(const Duration(days: 730))));
      });
    } else {
      try {
        data = await _requestHelper.getActiveHours(
  ...
  Future<void> _getAppointmentData() async {
    try {
      _loading.value = true;
      var product = await _requestHelper.getAppointmentProduct(
      ...
      _availabilityAutoSelect =
          get(product, ['availability_autoselect'], false);
      ...
      List staffIds = get(product, ['staff_ids'], []) is List
          ? get(product, ['staff_ids'], [])
          : [];

      dynamic data =
          await _getSlotsData(availabilityAutoSelect: _availabilityAutoSelect);
      ...
      if (staffAssignment == 'customer') {
        ...
        if (staffs.isNotEmpty) {
          for (var staff in staffs) {
            ...
            dynamic productStaff = products.firstWhereOrNull(
                (element) => '${get(element, ['id'], '')}' == widget.productId);

            double price = ConvertData.stringToDouble(
                get(productStaff, ['staff_cost'], ''));
            if (products.firstWhereOrNull((element) =>
                    '${get(element, ['id'], '')}' == widget.productId) !=
                null) {
              _listStaff
                  .add(StaffModel(id: idStaff, name: nameStaff, price: price));
            }
          }
          if (staffNopref) {
            if (mounted) {}
            _listStaff = [
              StaffModel(
                  name: AppLocalizations.of(context)!
                      .translate('product_appointment_no_preference')),
              ..._listStaff
            ];
          }
        }
  ...
  void initState() {
    _focusedDay.addListener(() {
      ...
      DateTime focusMonth =
          DateTime(_focusedDay.value.year, _focusedDay.value.month);
  ...
  Widget build(BuildContext context) {
    ...
    TextStyle defaultTextCalendar =
        theme.textTheme.subtitle2 ?? const TextStyle();
    TextStyle disableTextCalendar =
        theme.textTheme.bodyText2 ?? const TextStyle();
                        ...
                        if (!_timeStamps.value.contains(_pickedTime.value)) {
                          ...
                          widget.onChanged?.call({
                            ...
                            'date': DateFormat("yyyy-MM-dd")
                                .format(_pickedTime.value),
                            'time':
                                DateFormat("HH:mm").format(_pickedTime.value),
                            'staff_id': _pickedStaff.value.id,
                            'staff_name': _pickedStaff.value.name,
                          });
                        }
                    ...
                    builder: (context, value, _) {
                      return TableCalendar(
                        ...
                        calendarStyle: CalendarStyle(
                          ...
                          todayTextStyle: defaultTextCalendar.copyWith(
                              color: ColorBlock.red),
                          disabledTextStyle: disableTextCalendar,
                          selectedTextStyle: defaultTextCalendar.copyWith(
                        ...
                        onDaySelected: (selectedDay, focusedDay) {
                          if (!isSameDay(_selectedDay, selectedDay) &&
                            ...
                            if (_pickedTime.value != DateTime(1999)) {
                              _pickedTime.value = DateTime(
                                  selectedDay.year,
                                  selectedDay.month,
                                  selectedDay.day,
                                  _pickedTime.value.hour,
                                  _pickedTime.value.minute);
                              widget.onChanged?.call({
                                ...?widget.appointment,
                                'date': DateFormat("yyyy-MM-dd")
                                    .format(_pickedTime.value),
